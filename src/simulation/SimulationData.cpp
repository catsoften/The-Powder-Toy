...
std::vector<gol_menu> LoadGOLMenu()
{
	return std::vector<gol_menu>{
		...
		{String("LWDH"), PIXPACK(0xEE5BF2), 24, String("Life Without Death: B3/S012345678")},
		{String("SERV"), PIXPACK(0x37b129), 25, String("Serviettes: B234/S")},
		{String("VT45"), PIXPACK(0x7a8ad1), 26, String("Vote 4/5: B4678/S35678")},
		{String("FMZE"), PIXPACK(0x9db51f), 27, String("Flashing Maze: B24/S1234")},
		{String("LFOD"), PIXPACK(0x59bfec), 28, String("Live Free or Die: B2/S0")},
		{String("ANTI"), PIXPACK(0x7425f7), 29, String("Anti Life: B0123478/S01234678")},
		{String("INVL"), PIXPACK(0x9f18c3), 30, String("Inverse Life: B012345678/S34678")},
		{String("HTRE"), PIXPACK(0xcdf071), 31, String("H-Trees: B1/S012345678")},
		{String("IBAL"), PIXPACK(0xc9fcda), 32, String("Iceballs: B25678/S5678")},
		{String("RULH"), PIXPACK(0x7d066c), 33, String("Rule H: B2/S34")},
		{String("CRCF"), PIXPACK(0x5ac667), 34, String("Corrosion of Conformity: B3/S124")},
		{String("GEMS"), PIXPACK(0xce523d), 35, String("Gems: B3457/S4568")},
		{String("PLIF"), PIXPACK(0xae180f), 36, String("Pseudo GOL 2: B34/S23")},
		{String("DOTS"), PIXPACK(0x7bf916), 37, String("DotLife: B3/S023")}};
}
...
std::vector<std::array<int, 10> > LoadGOLRules()
{
	...
	//	 0,1,2,3,4,5,6,7,8,STATES    live=1  spawn=2 spawn&live=3   States are kind of how long until it dies, normal ones use two states(living,dead) for others the intermediate states live but do nothing
		...
		{1,1,1,3,1,1,1,1,1,2},//LWDH
		{0,0,2,2,2,0,0,0,0,2},//SERV
		{0,0,0,1,2,1,3,3,3,2},//VT45
		{0,1,3,1,3,0,0,0,0,2},//FMZE
		{1,0,2,0,0,0,0,0,0,2},//LFOD
		{3,3,3,3,3,0,1,3,3,2},//ANTI
		{2,2,2,3,3,2,3,3,3,2},//INVL
		{1,3,1,1,1,1,1,1,1,2},//HTRE
		{0,0,2,0,0,3,3,3,3,2},//IBAL
		{0,0,2,1,1,0,0,0,0,2},//RULH
		{0,1,1,2,1,0,0,0,0,2},//CRCF
		{0,0,0,2,3,3,1,2,1,2},//GEMS
		{0,0,1,3,2,0,0,0,0,2},//PLIF
		{1,0,1,3,0,0,0,0,0,2} //DOTS
	};
}
...
const BuiltinGOL builtinGol[NGOL] = {
	...
	// Bits B 87654321  Bits S 876543210
	{"LWDH", NGT_LWDH, 0b00000100111111111, PIXPACK(0xEE5BF2), PIXPACK(0xEE5BF2), NGT_LWDH, String("Life Without Death: B3/S012345678")},
	{"SERV", NGT_SERV, 0b00001110000000000, PIXPACK(0x37b129), PIXPACK(0x37b129), NGT_SERV, String("Serviettes: B234/S")},
	{"VT45", NGT_VT45, 0b11101000111101000, PIXPACK(0x7a8ad1), PIXPACK(0x7a8ad1), NGT_VT45, String("Vote 4/5: B4678/S35678")},
	{"FMZE", NGT_FMZE, 0b00001010000011110, PIXPACK(0x9db51f), PIXPACK(0x9db51f), NGT_FMZE, String("Flashing Maze: B24/S1234")},
	{"LFOD", NGT_LFOD, 0b00000010000000001, PIXPACK(0x59bfec), PIXPACK(0x59bfec), NGT_LFOD, String("Live Free or Die: B2/S0")},
	{"ANTI", NGT_ANTI, 0b11001111111011111, PIXPACK(0x7425f7), PIXPACK(0x7425f7), NGT_ANTI, String("Anti Life: B0123478/S01234678")},
	{"INVL", NGT_INVL, 0b11111111111011000, PIXPACK(0x9f18c3), PIXPACK(0x9f18c3), NGT_INVL, String("Inverse Life: B012345678/S34678")},
	{"HTRE", NGT_HTRE, 0b00000001111111111, PIXPACK(0xcdf071), PIXPACK(0xcdf071), NGT_HTRE, String("H-Trees: B1/S012345678")},
	{"IBAL", NGT_IBAL, 0b11110010111100000, PIXPACK(0xc9fcda), PIXPACK(0xc9fcda), NGT_IBAL, String("Iceballs: B25678/S5678")},
	{"RULH", NGT_RULH, 0b00000010000011000, PIXPACK(0x7d066c), PIXPACK(0x7d066c), NGT_RULH, String("Rule H: B2/S34")},
	{"CRCF", NGT_CRCF, 0b00000100000010110, PIXPACK(0x5ac667), PIXPACK(0x5ac667), NGT_CRCF, String("Corrosion of Conformity: B3/S124")},
	{"GEMS", NGT_GEMS, 0b01011100101110000, PIXPACK(0xce523d), PIXPACK(0xce523d), NGT_GEMS, String("Gems: B3457/S4568")},
	{"PLIF", NGT_PLIF, 0b00001100000001100, PIXPACK(0xae180f), PIXPACK(0xae180f), NGT_PLIF, String("Pseudo GOL 2: B34/S23")},
	{"DOTS", NGT_DOTS, 0b00000100000001101, PIXPACK(0x7bf916), PIXPACK(0x7bf916), NGT_DOTS, String("DotLife: B3/S023")}
};
...
std::vector<wall_type> LoadWalls()
{
	...
	std::vector<wall_type>{
		...
		{PIXPACK(0x006387), PIXPACK(0x000000), 2, Renderer::WallIcon, String("FARADAY WALL"),    "DEFAULT_WL_FARADAY", String("Blocks WIFI, ETRD, EMP, and portals inside a box of this wall.")},
...
std::vector<menu_section> LoadMenus()
{
	return std::vector<menu_section>{
		...
		{0xE056, String("Activated Materials"), 0, 1},
